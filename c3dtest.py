...
def get_sport_clip(clip_name, verbose = False):
    ...
    pdb.set_trace()
    clip = np.array([resize(io.imread(frame), output_shape = (112, 200), preserve_range = True, mode = 'constant') for frame in clip])
...
def main():
    ...
    X = get_sport_clip('roger') # (1, 3, 16, 112, 112)
    X = Variable(X)
    X = X.cuda()

    # get network pretrained model
    net = C3D()
    net.load_state_dict(torch.load(str(config.c3d_model_dir) + '/c3d.pickle'))
    net.cuda()
    net.eval()
